...
impl Default for AzureOpenAiBuilder {
    fn default() -> Self {
        Self {
            api_key: None,
            base_url: None,
...
impl AzureOpenAiProvider {
    ...
    pub fn from_env() -> AzureOpenAiBuilder {
        AzureOpenAiBuilder {
            api_key: std::env::var("AZURE_OPENAI_KEY").ok(),
            base_url: std::env::var("AZURE_OPENAI_ENDPOINT").ok(),
            model: std::env::var("AZURE_OPENAI_MODEL").ok(),
    ...
    fn request_body(&self, request: &CompletionRequest, stream: bool) -> serde_json::Value {
        let messages = request
            .messages
            .iter()
            .map(map_azureopenai_message)
            .collect::<Vec<_>>();
        let tools = request
            .tools
            .as_ref()
            .map(|tools| tools.iter().map(map_azureopenai_tool).collect::<Vec<_>>());
...
impl AzureOpenAiBuilder {
    ...
    /// Builds the provider.
    pub fn build(self) -> Result<AzureOpenAiProvider> {
        ...
        Ok(AzureOpenAiProvider {
            ...
            base_url: self
                .base_url
                .unwrap_or_else(|| "https://api.azureopenai.com/v1".to_string()),
...
impl LlmProvider for AzureOpenAiProvider {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse> {
        ...
        let payload = response
            ...
            .await
            .map_err(|error| ProviderError::Http(error.to_string()))?;
        map_azureopenai_response(payload)
    }
...
fn map_azureopenai_message(message: &Message) -> serde_json::Value {
    ...
    if let Some(tool_calls) = &message.tool_calls {
        value["tool_calls"] =
            serde_json::Value::Array(tool_calls.iter().map(map_azureopenai_tool_call).collect());
    }
...
fn map_azureopenai_tool(tool: &ToolDefinition) -> serde_json::Value {
    serde_json::json!({
        "type": "function",
        "function": {
            "name": tool.name,
...
fn map_azureopenai_tool_call(tool_call: &ToolCall) -> serde_json::Value {
    serde_json::json!({
        "id": tool_call.id,
        "type": "function",
        "function": {
...
fn map_azureopenai_response(payload: serde_json::Value) -> Result<CompletionResponse> {
    let choice = payload
        .get("choices")
